 ...
 * the user's machine and can need a password, so the button SEEDS Rudy's
...
function StatusChip({ tool }: { tool: ToolStatus }) {
  ...
  return (
    <span style={{
      fontFamily: 'var(--cth-font-ui)', fontWeight: 700, fontSize: 10.5, letterSpacing: 0.5,
...
export function SetupPanel({ onDone }: { onDone?: () => void } = {}) {
  ...
  // Only ESSENTIALS are handed to Rudy. Installing all eight engine CLIs
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 220, fontSize: 12, color: 'var(--cth-ink-700)', lineHeight: 1.5 }}>
          {missingEssential.length
            ? <>Rudy can install the {missingEssential.length} missing recommended {missingEssential.length === 1 ? 'tool' : 'tools'} for you. This fills in his dispatch box, nothing runs until you press dispatch.</>
        ...
        >
          <span style={{ display: 'inline-flex', gap: 4, alignItems: 'center' }}>
            <Icon name="sparkle" /> ask Rudy to set up everything
          </span>
        </PixelButton>
      </div>
