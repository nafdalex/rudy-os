...
//
// CONFORMED to the spec v1 (hive/docs/integrations-spec.md): the types are the
// CANONICAL ones from `@shared/integrations` (the src/shared/integrations.ts),
...
// test). The real path calls window.cth.* (the preload bridge).
//
// ⚠️ The preload bridge is NOT landed yet (owns it — boss relays when it's in).
// Until then this falls back to an in-memory mock so the UI is fully usable in dev.
// The real path is FEATURE-DETECTED: the moment the preload methods appear it
...
//      the preload must expose exactly these names so the detect matches.
//   2. The catalog served by `integrations:templates` is the `INTEGRATION_TEMPLATES`
//      (the 2 v1 reference templates). The richer src/shared/integrationTemplates.ts
//      is a SEPARATE, currently-unwired file — reconciliation is a orchestrator call.
//
...
// The preload bridge (Deliverable 2). Channels are fixed by §6;
...
// Serves the canonical INTEGRATION_TEMPLATES and validates upserts with the
