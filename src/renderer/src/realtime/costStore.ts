/**
 * Realtime Rudy — renderer cost store (card rt-9, cost-guard).
 *
 ...
 *
 * Net-new + disjoint by design: I own this file + the HUD that reads it. The
...
/** Begin metering a fresh session (call from session connect()). Preserves
...
/** Accumulate one usage delta (call on each realtime usage event). */
