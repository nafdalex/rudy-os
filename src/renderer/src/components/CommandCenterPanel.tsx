...
/** Rudy's control surface. Shown instead of the plain terminal/files panel
 *  when the boss agent is selected: terminal + queue, the floor roster (with
...
export type CCTab = 'terminal' | 'floor' | 'tasks' | 'human' | 'triggers' | 'trigger-history'
 ...
 *  Exported so the HQ bar draws the same eleven in the same order. */
export const CC_TABS: { key: CCTab; label: string; icon: Parameters<typeof Icon>[0]['name'] }[] = [
  { key: 'terminal', label: 'terminal', icon: 'terminal' },
  { key: 'floor', label: 'monitor', icon: 'mcp' },
  { key: 'tasks', label: 'tasks', icon: 'check' },
  { key: 'human', label: 'needs you', icon: 'bell' },
...
export function CommandCenterPanel({
  agent, fullscreen = false, hideTabs = false, hideHeader = false, terminalOnly = false,
  composerLeading, composerCompact = false, composerCollapsible = false
}: {
  agent: Agent;
  fullscreen?: boolean;
  /** Keep the header, drop the tab row (the shell drives the tabs). */
  hideTabs?: boolean;
  /** Drop the header strip too (the HQ desk draws its own). */
  hideHeader?: boolean;
  /** Pin the panel to the terminal and ignore external tab requests: in the
   *  HQ shell every other tab opens as a sheet drawn by the shell itself. */
  terminalOnly?: boolean;
  /** Passed through to the queue composer. */
  composerLeading?: ReactNode;
  composerCompact?: boolean;
  /** The queue folds to one line while it is empty; the terminal is the chat. */
  ...
  useEffect(() => {
    if (!ccTabRequest || terminalOnly) return;
  ...
  // control strips — toggling applies to every live agent, boss included.
  ...
  return (
    ...
    >
      {/* Header */}
      {!hideHeader && <div className="cc-head" style={{
      ...
      {!hideTabs && !terminalOnly && <div className="cth-tabbar" style={{
        display: 'flex', gap: 4,
        ...
        {visibleTabs.map((t) => (
          <button
            ...
            style={{
              ...
              padding: '6px 9px 5px', border: 'none', cursor: 'pointer',
              // Control voice, not colored chips: the filled-accent tabs were a
              // straight carry-over from the old app. Quiet text with a red
              // underline on the active tab is the same language as the rest
              // of the chrome.
              background: 'transparent',
              color: tab === t.key ? 'var(--cth-ink-900)' : 'var(--cth-ink-500)',
              boxShadow: tab === t.key ? 'inset 0 -2px 0 var(--cth-brand-red)' : 'none',
        ...
        {tab === 'terminal' && (
          isFullscreenedHere ? (
            ...
            <>
              ...
              <MessageQueueComposer agent={agent} leading={composerLeading} compact={composerCompact} collapsible={composerCollapsible} />
            </>
          ) : (
            <Centered>Rudy has no live terminal.</Centered>
          )
        )}
        {!terminalOnly && <CommandCenterTabBody agent={agent} tab={tab} onSwitchTab={setTab} />}
      </div>
    </PixelPanel>
  );
}
...
/**
 * Every Command Center tab except the terminal. Split from the panel so the
 * HQ shell can open any one of them as a sheet without mounting the panel's
 * chrome (or a second terminal on the same pty). Renders nothing for
 * 'terminal'; the terminal stays with whoever owns the pty.
 */
export function CommandCenterTabBody({ agent, tab, onSwitchTab }: {
  agent: Agent;
  tab: CCTab;
  onSwitchTab?: (tab: CCTab) => void;
}) {
  ...
  return (
    <>
      ...
      {tab === 'memory' && (
        <MemoryTab bossId={agent.id} who={selectedMemoryAgent ?? undefined} onWho={setSelectedMemoryAgent} />
      )}
      {tab === 'graph' && (
        <MemoryGraphPanel
          bossId={agent.id}
          onJumpToMemory={(id) => { setSelectedMemoryAgent(id); onSwitchTab?.('memory'); }}
        />
      )}
...
function FloorTab({ seed }: { seed: { text: string; seq: number } }) {
  ...
  // The harness's own default model (Settings → default model). Rudy and every
  ...
  const [dispatchTo, setDispatchTo] = useState<string>(''); // '' = Rudy decides
  ...
  useEffect(() => {
    window.cth.getConfig().then((c) => {
      ...
      setEngineProvider(c.bossProvider ?? 'claude');
      setEngineModel(c.bossModel);
  ...
  const restartWithModel = async (
    ...
    try {
      ...
      // opts.provider overrides the inferred provider — used when changing BOSS's engine.
  ...
  // ALL human dispatch flows through the boss — never directly into a worker's
  ...
  // picked in the dropdown is forwarded as a SUGGESTION the boss may follow.
  const dispatch = async () => {
    ...
    const res = await window.cth.hiveSend(
      { to: 'boss', act: 'request', subject: 'Task from the human', body: full },
      'human'
    );
    setDispatchText('');
    setDispatchMsg(res.ok
      ? `sent to Rudy${suggested ? ` (suggesting ${suggested.name})` : ''}`
      : `failed: ${res.error ?? '?'}`);
  ...
  const assignIssue = (issue: GHIssue) => {
    ...
    setDispatchTo(''); // Rudy decomposes and assigns — no more broadcast blasts
  };
  ...
  return (
    <Scroll>
      <Section title="HAND RUDY A JOB">
        <textarea
          value={dispatchText}
          onChange={(e) => setDispatchText(e.target.value)}
          rows={2}
          placeholder="Describe what you want done. Rudy breaks it down, writes the task card, and assigns it."
          style={textareaStyle}
        />
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6, flexWrap: 'wrap' }}>
          ...
          <span style={{ fontFamily: 'var(--cth-font-ui)', fontWeight: 700, fontSize: 10, color: 'var(--cth-ink-500)', flexShrink: 0 }}>
            WHO DOES IT
          </span>
          <Select value={dispatchTo} onChange={setDispatchTo}>
            <option value="">Rudy decides</option>
            {agents.filter((a) => !a.isBoss).map((a) => (
      ...
      <Section title="AGENTS">
        {agents.map((a) => {
          ...
          <div key={a.id} style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              {/* The standing answer to "what happens when the chat gets full":
                  it compacts itself. Printed here, at the meter people watch. */}
              <span style={{ flex: 1, fontFamily: 'var(--cth-font-mono)', fontSize: 10, color: 'var(--cth-ink-300)', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}
                title="When this meter fills past the threshold, the app sends /compact to this agent: the chat is summarized in place and the window frees up. Tune it under TRIGGERS, CONTEXT.">
            ...
            {/* Non-boss agents get the cross-provider model picker + restart controls
                here. The BOSS agent's model lives in the engine row below
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
              ...
              >
                ...
                {modelProvidersForAgent(a.isBoss).map((preset) => (
            ...
            {a.isBoss && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                ...
                <PixelButton
                  ...
                  onClick={async () => {
                    ...
                    if (engineProvider !== currentProvider) {
                      if (!window.confirm("This restarts Rudy; a conversation on a different engine can't be resumed.")) return;
                    }
                ...
                >
                  {restarting === a.id ? 'switching…' : 'switch'}
                </PixelButton>
                {/* Redraw a garbled terminal without losing the thread (resume the
                    SAME engine+model). Kept here since the boss has no per-agent row above. */}
                <PixelButton
                  variant="secondary"
                  size="sm"
                  disabled={restarting === a.id}
                  onClick={() => restartWithModel(a, a.model, { resume: true })}
                >
                  <span title="Kill and respawn Rudy, resuming the current conversation, fixes a corrupted/garbled terminal without losing context">
                    restart &amp; continue
                  </span>
                </PixelButton>
              </div>
        ...
        <div style={{
          ...
          <span><strong>{fmtTokens(sumTokens)}</strong> tokens total</span>
          <span style={{ color: 'var(--cth-ink-700)' }}>{fmtTokens(sumInput)} in, {fleetCachePct}% from cache</span>
          <span style={{ color: 'var(--cth-ink-700)' }}>{Math.round(sumRate).toLocaleString()} tok/min now</span>
        </div>
        <div style={{ marginTop: 6 }}>
          <Muted>
            Live usage. Each bar compares tokens used against that agent&apos;s cap, or the {fmtTokens(floorCap)} floor budget when no cap is set
            {tokenCap && tokenCap > 0 ? '.' : ' (change it in Settings).'}
          </Muted>
        </div>
      </Section>
      ...
      <Section title="PROJECTS">
        {repos.length === 0 && (
          <Muted>No projects yet. Add one while hiring an agent (Workspace step) and it shows up here.</Muted>
        )}
        {repos.map((r) => (
      ...
      {/* GitHub issues only exist once there is a project to fetch them from —
          an empty twin of the empty section above was pure noise. */}
      {repos.length > 0 && (
    ...
    <Section title={`ARCHIVED (${archivedAgents.length})`}>
      ...
      {open && archivedAgents.map((a) => (
        <div key={a.id} style={{
          ...
          <div style={{
            width: 24, height: 24, background: '#F2ECE4', borderRadius: '27%',
            boxShadow: 'inset 0 0 0 1px var(--cth-ink-300)',
            display: 'flex', alignItems: 'flex-start', justifyContent: 'center', overflow: 'hidden', flexShrink: 0
          }}>
            <SpritePortrait character={a.character} scale={1} />
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
...
function MemoryTab({ bossId, who: controlledWho, onWho }: { bossId: string; who?: string; onWho?: (id: string) => void }) {
  const agents = useStore((s) => s.agents);
  // Selection is controllable from the graph tab; falls back to local state.
  const [internalWho, setInternalWho] = useState<string>(bossId);
    ...
    <Scroll>
      <Section title="HOW MEMORY WORKS">
        <div style={{
          ...
          Always on, no button to press: every agent reads its own memory file when
          it starts a task and appends what it learned when it finishes. In the
          background the harness condenses each file on a rolling cadence so they
          stay sharp instead of growing forever (Settings, Memory &amp; Knowledge),
          and mines them into the shared search below.
        </div>
      </Section>
...
/** Event kind → chip colour. Same hue language as the floor's envelopes. */
function activityTone(kind?: string): string {
  switch (kind) {
    case 'spawn': return 'var(--cth-mint)';
    case 'message': return 'var(--cth-sky)';
    case 'drain': return 'var(--cth-lilac)';
    case 'escalate': return 'var(--cth-coral)';
    case 'approval': return 'var(--cth-lemon)';
...
function ActivityTab() {
  ...
  // Ids → human names, so the ledger reads as people ("Rudy → Theo"), falling
  // back through the restorable roster for workers that already left the floor.
  const nameOf = (id: unknown): string => {
    if (typeof id !== 'string' || !id) return '?';
    if (id === 'human') return 'you';
    if (id === 'broadcast') return 'everyone';
  ...
  const fmt = (e: LogEntry): string => {
    switch (e.kind) {
      case 'spawn': return `${e.name ?? nameOf(e.agentId)} joined the floor`;
      case 'message': return `${nameOf(e.from)} → ${nameOf(e.to)}: ${e.subject || e.act || 'message'}`;
      case 'drain': return `${nameOf(e.agentId)} read ${e.count} inbox message${(e.count as number) === 1 ? '' : 's'}`;
      case 'escalate': return `escalated to you: ${e.subject ?? ''}`;
      case 'approval': return `you ${e.approve ? 'approved' : 'declined'} a request`;
      default: {
        // Unknown kinds still deserve a sentence, not raw JSON.
        const rest = Object.entries(e)
          .filter(([k]) => k !== 'kind' && k !== 'ts')
          .map(([k, v]) => `${k}: ${typeof v === 'string' ? v : JSON.stringify(v)}`)
          .join(', ');
        return rest.length > 90 ? rest.slice(0, 89) + '…' : rest || 'event';
      }
    }
  };
  ...
  return (
    <Scroll>
      <Section title="ACTIVITY · WHAT RUDY HAS BEEN ORCHESTRATING">
        {log.length === 0 && <Muted>Nothing yet. The ledger fills as the floor works.</Muted>}
        {[...log].reverse().map((e, i) => (
          <div key={i} style={{
            ...
            <span style={{
              flexShrink: 0, width: 34, fontFamily: 'var(--cth-font-mono)',
              fontSize: 10, color: 'var(--cth-ink-500)'
            }}>{clock(e.ts)}</span>
            <span style={{
              flexShrink: 0, width: 7, height: 7, alignSelf: 'center',
              background: activityTone(e.kind), boxShadow: 'inset 0 0 0 1px var(--cth-ink-300)'
            }} title={e.kind ?? 'event'} />
            <span style={{
      ...
      <Section title="THE BOARD · RUDY'S OWN PLAN, IN HIS WORDS">
        <Pre>{board || 'The board is empty. Rudy writes it as he plans.'}</Pre>
      </Section>
    </Scroll>
  );
}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 14 }}>
      <div style={{ fontFamily: 'var(--cth-font-ui)', fontWeight: 700, fontSize: 10, lineHeight: '12px', letterSpacing: '0.06em', color: 'var(--cth-ink-500)', marginBottom: 6 }}>{title}</div>
      {children}
    </div>
  );
}
