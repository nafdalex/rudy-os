/**
 * Realtime Rudy — read-tools (rt-4, Realtime Rudy Phase 1).
 *
 ...
 * allowlist only.
 *
 * INTEGRATION (rt-2's src/renderer/src/realtime/session.ts — the file):
...
export function realtimeReadTools(): ReturnType<typeof tool>[] {
  return [
    ...
    tool({
      ...
      description:
        'Who is in the agent hive right now: how many agents, which are active versus archived, who the boss orchestrator is, and each active agent name, role, and engine. Call this when the user asks who is working, who is on the floor, or for a roster.',
      ...
      execute: () =>
        spoken(async () => {
          ...
          const bossId = reg.bossId;
          const bossName = bossId ? str(obj(obj(reg.agents)[bossId]).name) || bossId : null;
          const lines = active
            .filter(([id]) => id !== bossId)
            .map(([, a]) => {
              const m = obj(a);
          ...
          const boss = bossName ? ` ${bossName} is the boss orchestrator.` : '';
          const roster = lines.length ? ` Active workers: ${lines.join('; ')}.` : '';
          return head + boss + roster;
        }, 'fleet status')
    }),
    ...
    tool({
      ...
      execute: () =>
        spoken(async () => {
          ...
          if (c.bossProvider || c.bossModel)
            parts.push(`The boss orchestrator runs ${[c.bossProvider, c.bossModel].filter(Boolean).join(' ')}.`);
    ...
    tool({
      ...
      parameters: {
        type: 'object',
        properties: {
          agentId: { type: 'string', description: 'The agent id or friendly name to look up (e.g. "kofi-mqpbq43v" or "Kofi").' }
        },
        required: ['agentId'],
        additionalProperties: false
      },
    ...
    tool({
      name: 'get_app_info',
      description:
        'About the Rudy OS app itself: the running version and the latest release notes (changelog). Use for "what version is this" or "what is new in this release".',
      parameters: { type: 'object', properties: {}, required: [], additionalProperties: false },
      execute: () =>
        spoken(async () => {
          const info = await window.cth.appInfo();
          const notes = despan(info.changelog || '');
          return `This is Rudy OS version ${info.version}. ${notes ? `Latest release notes: ${clip(notes, 1600)}` : 'No release notes are bundled with this build.'}`;
        }, 'app info')
    })
  ];
}
...
/**
 * A short, preloaded orientation Rudy can open the session with — the hive
 * size, who boss is, and how many tasks are in flight — so the first answer is
...
export async function realtimeSessionSummary(): Promise<string> {
  try {
    ...
    const bossRow = rows.find((a) => a.isBoss === true);
    ...
    return (
      ...
      `${bossRow ? `, ${str(bossRow.name)} orchestrating alongside you` : ''}. ` +
      ...
      ` You share the floor with boss (the typing orchestrator); the board is the single source of truth.`
    );
  } catch {
    return '';
  }
