...
export function GitTab({ cwd }: GitTabProps) {
  ...
  if (isRepo === false) {
    // The text sits in an inner block: putting it directly in the flex
    // container made each text node its own flex item, which is what mangled
    // the line into "Run git initin the agent's terminal".
    return (
      <div style={{
        flex: 1, minWidth: 0,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, textAlign: 'center' }}>
          <span style={{
            ...
            letterSpacing: '0.05em', textTransform: 'uppercase', color: 'var(--cth-ink-500)'
          }}>
            Not a git repository
          </span>
          <span style={{ fontFamily: 'var(--cth-font-ui)', fontSize: 12.5, lineHeight: '18px', color: 'var(--cth-ink-500)' }}>
            This folder has no version history yet. Run{' '}
            <code style={{
              fontFamily: 'var(--cth-font-mono)', padding: '1px 5px',
              background: 'var(--cth-cream-100)', boxShadow: 'inset 0 0 0 1px var(--cth-ink-100)',
              color: 'var(--cth-ink-900)'
            }}>git init</code>{' '}
            in the agent's terminal to start one.
          </span>
        </div>
      </div>
    );
