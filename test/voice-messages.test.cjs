...
// ── fixtures ─────────────────────────────────────────────────────────────────
const mk = (over) => ({
  id: 'm1', conversation: 'c1', in_reply_to: null, from: 'kofi', to: 'boss',
...
const LOG = [
  { ts: 1, kind: 'spawn', agentId: 'mara' },
  { ts: 2, kind: 'message', from: 'kofi', agentId: 'kofi' },
  { ts: 3, kind: 'tasks', count: 12 },
  { ts: 4, kind: 'archive', agentId: 'wren' },
  { ts: 5, kind: 'voice_action', actor: 'rudy-voice' }
];
...
// ===== MESSAGE RETRIEVAL =====================================================
const A = mk({ id: 'a', from: 'kofi', to: 'boss', subject: 'done', body: 'shipped', created_at: '2026-06-27T08:00:00.000Z' });
const B = mk({ id: 'b', from: 'boss', to: 'kofi', subject: 'next', body: 'pick up card x', created_at: '2026-06-27T09:30:00.000Z' });
const C = mk({ id: 'c', from: 'ines', to: 'boss', subject: 'review', body: 'looks good', created_at: '2026-06-27T07:00:00.000Z' });
...
const TAGGED = [
  { msg: B, owner: 'kofi', direction: 'inbox', archived: false },
  { msg: A, owner: 'kofi', direction: 'outbox', archived: true },
...
test('shape: carries direction + owner + archived for briefing context', () => {
  const r = selectMessages(TAGGED, { id: 'a' });
  assert.strictEqual(r[0].owner, 'kofi');
...
const SECRETS = [
  ...
  // Ines hardening: a LABELED aws secret access key — the AKIA id half is caught
...
const BENIGN = [
  ...
  'kofi-mqpbq43v parked, awaiting assignment',
  '/Users/tester/Documents/Personal/cth-voice-msg-access is the worktree',
  'The token cap is 1.2 million tokens this session.',
  'Tasks: 3 todo, 1 doing, 0 blocked, 12 done.',
  'Ines approved 8 of 8 dimensions, no must-fix.'
];
