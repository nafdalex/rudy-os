...
const TASKS = [
  ...
  { id: 'rt-2-realtime-session', title: 'Realtime Rudy 2: WebRTC session — mic capture, VAD, barge-in', status: 'done' },
  { id: 'rt-3-toggle-ui', title: 'Realtime Rudy 3: mic toggle + live state dot on AgentCard', status: 'done' },
...
// 6. ambiguity: "realtime rudy" matches rt-2 AND rt-3 closely -> must flag, not guess
test('ambiguous: "realtime rudy" flags disambiguation (no silent write)', () => {
  const r = find('realtime rudy');
