...
export function AiEnginesSettings({ config }: { config: HarnessConfig }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {/* Per-CLI local endpoint + default model. Skippable for most people;
          say so up front instead of leading with jargon. */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div style={headStyle}>RUN ON YOUR OWN MODELS (OPTIONAL)</div>
        <div style={{ fontSize: 12, color: 'var(--cth-ink-700)', lineHeight: '18px' }}>
          Only for running models on your own machine or server (Ollama, LM Studio, a company
          endpoint). Leave everything here blank if your engines use API keys above.
          Per engine: the <strong>server address</strong> it should talk to, and the
          <strong> model</strong> it should use there.
        </div>
        {CLIS.map((c) => (
          <div key={c.id} style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              <input
                placeholder={c.hint}
                title={`Server address ${c.label} should talk to. ${c.hint}`}
                defaultValue={baseUrls[c.id] ?? ''}
                onBlur={(e) => saveBaseUrl(c.id, e.target.value)}
                style={inputStyle}
              />
              <input
                placeholder="model to use, e.g. ollama/llama3.3"
                title={`The model ${c.label} runs by default, written provider/model`}
      ...
      {/* Unsandboxed-in-auto caveat (guardrail #6) */}
      <div style={{
