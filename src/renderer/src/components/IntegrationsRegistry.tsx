...
// Integrations configuration UI — Settings → Integrations. Conformed to the
// spec v1 (hive/docs/integrations-spec.md) and styled to the mockup
...
//
// v1 worker model (§4): the broker grants EVERY enabled integration to ALL
...
// extension; this reflection updates once that model is confirmed.
...
// UI-only brand glyphs (the templates carry no glyph). Falls back to label initials.
...
const dispLabel: CSSProperties = { fontFamily: 'var(--cth-font-ui)', fontWeight: 700, fontSize: 10, lineHeight: '12px', color: 'var(--cth-ink-500)', textTransform: 'uppercase' };
...
/** Is an integration actually usable by workers? (§6 gate.) */
...
export function IntegrationsRegistry() {
  ...
  const validate = (d: Draft): string | null => {
    ...
    if (d.authType === 'header' && !/^[A-Za-z0-9-]{1,64}$/.test(d.authHeader.trim())) return 'Header name must be 1 to 64 chars of A to Z, 0 to 9, or "-".';
    return null;
  };
            ...
            return (
              <button key={t.idSuggestion} type="button" onClick={() => setPicked(t.idSuggestion)} style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 2, minWidth: 0, overflowWrap: 'anywhere' }}>
                  <span style={{ fontFamily: 'var(--cth-font-ui)', fontWeight: 700, fontSize: 10.5, lineHeight: '15px', letterSpacing: '0.05em', textTransform: 'uppercase', color: 'var(--cth-ink-900)' }}>{t.label}</span>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 12, background: 'var(--cth-cream-100)', boxShadow: 'inset 0 0 0 1px var(--cth-ink-300)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <span style={{ fontFamily: 'var(--cth-font-ui)', fontWeight: 700, fontSize: 11.5, lineHeight: '16px', letterSpacing: '0.05em', textTransform: 'uppercase', color: 'var(--cth-ink-900)' }}>{tpl?.label ?? draft.kind}</span>
                ...
                <div key={r.id} style={{ display: 'flex', flexDirection: 'column', gap: 6, padding: 10, background: 'var(--cth-paper-100)', boxShadow: 'inset 0 0 0 1px var(--cth-ink-300)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 2, flex: 1, minWidth: 0 }}>
                      <span style={{ fontFamily: 'var(--cth-font-ui)', fontWeight: 700, fontSize: 10.5, lineHeight: '15px', letterSpacing: '0.05em', textTransform: 'uppercase', color: 'var(--cth-ink-900)' }}>{r.label}</span>
