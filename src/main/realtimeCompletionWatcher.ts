/**
 * Realtime Rudy — completion watcher (card rt-12, Phase 2, "respond when done").
 *
 * When voice-Rudy DISPATCHES work (fire-and-notify, the default pattern), he does
 ...
 * make Rudy speak it unprompted ("Theo finished — want details?").
 *
 * OWNERSHIP / SEAM (rt-12 split, boss-ruled 2026-06-25): this module owns the watcher and
 * is deliberately DISJOINT from the realtime CORE files. It NEVER imports session.ts
 * / the live RealtimeSession / electron — it only takes injected readers + a clock and
 * EMITS via callbacks. The side subscribes `onCompletion(...)` and pushes the event
...
/** A unit of work voice-Rudy dispatched and is now awaiting completion of. */
 ...
 * `drainQueuedCompletions()` — the SAME shape (rt-12 contract lock). The renderer
 * forwards it verbatim over the main→renderer push channel and into warm-start, so every
 * field here reaches Rudy. `summary` is the human-speakable line; `completedAt` is
...
export interface RealtimeCompletion {
  ...
  targetAgentId: string;
  taskId?: string;
  /** Human-speakable line, e.g. "Theo finished the cost guard." */
  summary: string;
 ...
 * whitespace, and cap length so a malicious objective can't steer what Rudy says or does.
 * (also neutralizes at the session.ts injection seam — this is the watcher-half belt.)
 */
...
function speakableName(agentId: string): string {
  // "theo-mqpbr18v" → "Theo". Falls back to the raw id if it has no name segment.
 ...
 * warm-start. It owns NO realtime/session/electron state — the core wires the emit to
...
// --- shared singleton (rt-12 contract lock) ------------------------------------
