...
interface ScheduledMission {
  ...
  /** 'rudy' = the boss filed this via trigger-requests/ (main/triggerRequests.ts). */
  origin?: 'rudy';
}
...
export function SchedulesSection({ onSummary }: { onSummary?: (s: string) => void }) {
  ...
  const [mTo, setMTo] = useState<string>('boss');
  ...
  const targetName = (to: string) =>
    to === 'broadcast' ? 'everyone' : to === 'boss' ? 'Rudy' : agents.find((a) => a.id === to)?.name ?? to;
  ...
  return (
    <>
      ...
      {adding && (
        <SubCard>
          <div style={{ fontFamily: 'var(--cth-font-ui)', fontWeight: 700, fontSize: 10, color: 'var(--cth-ink-500)' }}>NEW SCHEDULE</div>
          <Field label="LABEL">
            <input
              value={mLabel}
          ...
          <Field label="GOES TO">
            <Select value={mTo} onChange={setMTo} style={{ width: '100%' }}>
              ...
              <option value="boss">Rudy</option>
              {agents.filter((a) => !a.isBoss).map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}
            </Select>
          </Field>
          <Field label="WHEN">
            <SchedulePicker
...
interface RosterAgent { id: string; name: string; isBoss?: boolean }
      ...
      <SubHeader
        ...
        title={
          <span style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 0 }}>
            ...
            {mission.origin === 'rudy' && (
              <span
                title="Rudy filed this schedule himself, from the terminal. Edit or switch it off like any other."
                style={{
                  flexShrink: 0, padding: '2px 5px 1px',
                  fontFamily: 'var(--cth-font-ui)', fontWeight: 700, fontSize: 9.5, letterSpacing: '0.06em',
                  color: 'var(--cth-sky)', boxShadow: 'inset 0 0 0 1px var(--cth-sky)'
                }}
              >BY RUDY</span>
            )}
