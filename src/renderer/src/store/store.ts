...
export interface Agent {
  ...
  lastPrompt?: string;
  /** the orchestrator ("boss") agent — seated in Rudy's room, runs the floor */
  isBoss?: boolean;
  /** Rudy's prep assistant — send-only; enriches prompts and forwards them to
   *  the boss. Excluded from broadcast fan-out and from the restorable-dead sweep. */
  isAssistant?: boolean;
  /** The human has this agent 1:1 and Rudy has been told to leave it alone.
...
export interface QueuedMessage {
   ...
   *  `text`. Used by Slack-origin work to carry the autonomy preamble to boss's
...
/** Lifecycle of the boss agent ("Rudy") bootstrap on launch.
 ...
 *  don't see the "add agent" prompt before Rudy has clocked in. */
export type BossStatus = 'booting' | 'ready' | 'failed';
...
interface State {
   ...
   *  registry entry reattach by themselves. Boss/assistant are excluded (they
   *  auto-respawn). */
  ...
  /** Clear an archived agent from the renderer lists (the hive registry keeps
   *  its record). The id is remembered so reconciliation does not re-add it. */
  removeArchivedAgent: (id: string) => void;
  /** Re-add archived workers the hive registry knows but the renderer lost
   *  (mis-click, wiped storage). Skips live/known/dismissed ids. */
  restoreArchivedFromHive: (entries: Agent[]) => void;
   ...
   *  secret broker, never the store). Gates the Realtime Rudy voice toggle the
...
/** Archived entries the user cleared from the floor list — WITHOUT this the
 *  hive reconciliation would resurrect every cleared card on the next boot. */
const LS_DISMISSED_ARCHIVED = 'cth.dismissedArchived';
...
export const useStore = create<State>((set, get) => ({
  ...
  setBossStatus: (status) => set({ bossStatus: status }),
    ...
    set((s) => {
      const apply = (list: Agent[]): Agent[] => {
        ...
        const next = list.map((a) => {
          const description = preferredAgentRole(a.description, roles[a.id], !!a.isBoss);
  ...
  addAgent: (agent) =>
    set((s) => {
      ...
      // BOSS enters at the HEAD, everyone else at the tail. Rudy's position was
      // otherwise decided by a race he usually lost: useHive's bootstrap removes
      // the restored boss entry, then spawns him asynchronously (a setTimeout, a
      ...
      // order, and a boss-first sort at render time would silently override the
      ...
      const agents = agent.isBoss ? [agent, ...s.agents] : [...s.agents, agent];
  ...
  removeArchivedAgent: (id) =>
    set((s) => {
      ...
      persistArchived(archivedAgents);
      // Remember the dismissal, or restoreArchivedFromHive would bring the
      // card straight back on the next boot.
      try {
        const seen: string[] = JSON.parse(localStorage.getItem(LS_DISMISSED_ARCHIVED) ?? '[]');
        if (!seen.includes(id)) localStorage.setItem(LS_DISMISSED_ARCHIVED, JSON.stringify([...seen, id]));
      } catch { /* storage unavailable — the card may return next boot */ }
      return { archivedAgents };
    }),
  restoreArchivedFromHive: (entries) =>
    set((s) => {
      // The hive registry is the durable truth about who was archived. Anyone
      // it knows that the renderer lost (a mis-click on clear, a wiped
      // localStorage) comes back as a clocked-out card — unless the user
      // deliberately dismissed that id.
      let dismissed: string[] = [];
      try { dismissed = JSON.parse(localStorage.getItem(LS_DISMISSED_ARCHIVED) ?? '[]'); } catch { /* none */ }
      const known = new Set([...s.agents, ...s.archivedAgents, ...s.restorableAgents].map((a) => a.id));
      const fresh = entries.filter((e) => !known.has(e.id) && !dismissed.includes(e.id));
      if (fresh.length === 0) return s;
      const archivedAgents = [...s.archivedAgents, ...fresh];
      persistArchived(archivedAgents);
      return { archivedAgents };
    }),
  ...
  enqueueMessage: (agentId, text, meta) =>
    set((s) => {
      ...
      // several — the context trigger, boss dispatching a work order, Slack, the
  ...
  reconcileWithLivePtys: (livePtyIds) =>
    set((s) => {
      ...
      // (full spawn recipe retained) instead of silently vanishing. Boss and the
      // prep assistant are excluded — they auto-respawn at boot.
      const dead = s.agents.filter(
        (a) => a.ptyId && !live.has(a.ptyId) && !a.isBoss && !a.isAssistant
      );
