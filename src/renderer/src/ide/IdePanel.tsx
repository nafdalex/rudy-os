 ...
 *   3. the boss agent, then the first agent — last resorts so the IDE still
...
function pickIdeTarget(): IdeTarget {
  ...
  const guess = byId(s.selectedId) ?? s.agents.find((a) => a.isBoss) ?? s.agents[0] ?? null;
...
export function IdePanel() {
  ...
  return (
    <div style={{
      ...
      {/* Title bar */}
      <div
        className="cth-titlebar-drag cth-darkband"
        style={{
      ...
      >
        {/* The one true wordmark (SVG geometry) — a Press Start 2P "RUDY OS"
            here read as a second, broken-looking logo. */}
        <BrandLogo height={13} />
        <span style={{
          fontFamily: 'var(--cth-font-ui)', fontWeight: 700, fontSize: 10.5,
          letterSpacing: '0.14em', color: 'var(--cth-ink-500)'
        }}>
          IDE
        </span>
        {/* The band is the brand and nothing else — the agent + workspace text
            that used to sit here read as clutter next to the wordmark. Whose
            files these are lives on hover, and in the tree itself. */}
        <span
          title={target.agent ? `${target.agent.name}'s workspace${root ? `: ${root}` : ''}` : (root ?? '')}
      ...
      {!root ? (
        <div style={{
          ...
          textAlign: 'center', color: 'var(--cth-ink-500)', fontFamily: 'var(--cth-font-ui)', fontSize: 12.5
        }}>
              ...
              {(['changes', 'history', 'compare'] as const).map((k) => (
                <button
                  ...
                  style={{
                    ...
                    fontFamily: 'var(--cth-font-ui)', fontWeight: 700, fontSize: 10, lineHeight: '14px',
              ...
              {tabs.map((t) => {
                ...
                return (
                  ...
                  >
                    {t.mode !== 'edit' && (
                      <span style={{
                        fontFamily: 'var(--cth-font-ui)', fontWeight: 700, fontSize: 9.5, padding: '1px 3px',
            ...
            <div style={{ flex: 1, minHeight: 0, position: 'relative' }}>
              {!activeTab && (
                <div style={{
                  ...
                  <div style={{
                    fontFamily: 'var(--cth-font-ui)', fontWeight: 700, fontSize: 10, textTransform: 'uppercase',
...
function SectionHeader({ title, right }: { title: string; right?: React.ReactNode }) {
  return (
    <div style={{
      ...
      fontFamily: 'var(--cth-font-ui)', fontWeight: 700, fontSize: 10, lineHeight: '12px', textTransform: 'uppercase',
