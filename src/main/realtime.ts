/**
 * Realtime Rudy — main-process ephemeral-token mint (card rt-1, Phase 1).
 *
 ...
 *  line (rt-1 COORD: index.ts is edited in the same lane). */
 ...
 *  Realtime Rudy voice toggle in the renderer, the way `hasGroqKey` gates the
 ...
 *  one line — rt-1 COORD note (also edits index.ts). Neither handler ever
