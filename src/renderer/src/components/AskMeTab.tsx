 ...
 *
 * Tasks the boss can only move with the human's input sit here. An entry isn't
 ...
 *   2. mails the boss so it picks the answer up, unblocks the card, and the
...
export function AskMeTab() {
   ...
   * writing our snapshot back would delete any card the boss added since the last
   * poll. Re-locating the open question by its text also means an answer can
   * never land on a different question the boss swapped in underneath us — in
  ...
  const sendAnswer = async (task: HiveTask) => {
    ...
    try {
      ...
      setTasks(next);
      // 2) Tell the boss, so the card gets unblocked and work continues.
  ...
  // blocked on the kanban; the boss can re-ask by appending a fresh humanQA entry.
  ...
  return (
    // Body text is set in the mono face (JetBrains Mono) — the same readable font the
    // memory viewer uses. The display face (font-display) is too chunky for prose like
        ...
        return (
          <div key={t.id} style={{
            ...
            <div style={{
              ...
              <button
                ...
                style={{
                  ...
                  fontFamily: 'var(--cth-font-mono)', fontSize: 12.5, fontWeight: 600, color: 'var(--cth-ink-900)',
            ...
            <div style={{ padding: 9, display: 'flex', flexDirection: 'column', gap: 8 }}>
              {/* the question */}
              <div style={{ fontSize: 12.5, lineHeight: '18px', color: 'var(--cth-ink-900)', whiteSpace: 'pre-wrap' }}>
                {open.q}
              </div>
              ...
              <textarea
                ...
                style={{
                  ...
                  fontFamily: 'var(--cth-font-mono)', fontSize: 12.5, lineHeight: '18px',
              ...
              {stuck.length > 0 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                  <div style={{ fontFamily: 'var(--cth-font-ui)', fontWeight: 700, fontSize: 10, color: 'var(--cth-coral)' }}>
