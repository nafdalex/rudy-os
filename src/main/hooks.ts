...
export class HookServer {
  ...
  private handle(p: HookPayload): unknown {
    ...
    if ((event === 'Stop' || event === 'SubagentStop') && agentId) {
      ...
      this.notify(this.displayName(agentId), 'finished, idle');
      this.emit(agentId, event, p);
      return {};
    }
    ...
    // Keep boss's roster CURRENT. fleet.json is always fresh on disk, but boss's
    ...
    // additionalContext at the start of each session and on every prompt, so boss
    // knows the floor all the time instead of only when it remembers to Read.
    // Boss-only and one line — every other agent is unaffected.
    const wantsRoster = (event === 'SessionStart' || event === 'UserPromptSubmit')
      && !!agentId && this.hive.isBoss(agentId);
    // Hand the roster the LIVE context-window occupancy (contextById) so each
    // agent line can carry a `ctx NN%` — boss then sees whose context is nearly
    ...
    if (
      ...
      this.notify(this.displayName(agentId), p.message ?? 'needs your attention');
    }
  ...
  /** Toast title people can read. The registry name when the id is hired
   *  (worker-milo → Milo); the id itself when it is already a readable slug;
   *  and "Rudy's helper" for opaque session hashes — Claude's inner subagents,
   *  which never had a name anywhere. */
  private displayName(agentId: string | undefined): string {
    if (!agentId) return 'Agent';
    try {
      const name = this.hive.registry().agents[agentId]?.name;
      if (name) return name;
    } catch { /* registry unreadable — fall through to the id */ }
    return /^[0-9a-f][0-9a-f-]{11,}$/i.test(agentId) ? "Rudy's helper" : agentId;
  }
