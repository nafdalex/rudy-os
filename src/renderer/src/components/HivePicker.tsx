...
export function HivePicker({ config, onOpenCurrent }: HivePickerProps) {
  ...
  return (
    <div style={{
      ...
      backgroundImage: `radial-gradient(var(--cth-grid-dot) 1px, transparent 1px)`,
      backgroundSize: '22px 22px',
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      zIndex: 200,
      padding: 32
    }}>
      {/* Dark band under the macOS traffic lights, same as every titlebar. */}
      <div className="cth-darkband" style={{ position: 'fixed', top: 0, left: 0, right: 0, height: 36, background: '#141414', borderBottom: '1px solid #2E2E2E' }} />
      <div style={{ width: 560, maxWidth: '94vw' }}>
        <PixelPanel variant="dialog" title="SELECT A HARNESS CONFIG" noPadding>
          <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {current && (
              <div>
                <div style={{ fontFamily: 'var(--cth-font-ui)', fontWeight: 700, fontSize: 10.5, color: 'var(--cth-ink-500)', marginBottom: 4 }}>
                  CURRENT
                </div>
                <div style={{
            ...
            {/* ONE browse button: "open existing" and "create new" were the
                same folder dialog twice, which read as a broken choice. Picking
                an empty folder bootstraps it; picking a used one opens it. */}
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', flexWrap: 'wrap' }}>
              <PixelButton variant="secondary" size="md" onClick={browse} disabled={!!busy}
                title="Pick a folder. An empty folder becomes a fresh config; a folder with an existing config opens as it is.">
                <span style={{ display: 'inline-flex', gap: 6, alignItems: 'center' }}>
                  <Icon name="folder" /> open or create a config folder…
                </span>
              </PixelButton>
            </div>
          </div>
