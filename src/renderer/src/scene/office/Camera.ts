...
export class Camera {
  ...
  private getMinZoom(): number {
    ...
    const fit = Math.min(this.viewWidth / this.mapWidth, this.viewHeight / this.mapHeight);
    if (fit >= 1) return fit;
    // JUST under 1:1 (the grown sheet sized to the map, minus a border or
    // two): take 1:1 anyway. A few edge pixels hidden beats the halving rule
    // slamming a 0.98 fit down to a half-size map in an empty sheet.
    if (fit >= 0.9) return 1;
    // Genuinely small (the postcard): snap to halvings (1/2, 1/4 …). An
    // arbitrary fit like 0.28 makes nearest sampling drop pixel rows
    // UNEVENLY — the muddy stutter. A power-of-two step decimates uniformly
    // (every 2x2 block → one pixel), clean deliberate pixel art with the
    // whole office in frame.
    let z = 0.5;
    while (z > fit && z > 1 / 32) z /= 2;
    return z;
  }
