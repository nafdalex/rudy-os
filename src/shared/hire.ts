 ...
 * `rudyos://hire?src=<https-url>` deep link or an in-app file picker.
 *
...
export const HIRE_SPEC_V1 = 'rudy-os/hire@1';
...
export interface HireManifest {
  /** Spec tag; exactly `rudy-os/hire@1` for this version. */
  ...
  goal?: string;
  /** Cast sprite id (e.g. 'noor'); unknown values fall back to default. */
  character?: string;
...
/** Parse a `rudyos://hire?src=<https-url>` deep link. Returns the https
...
export function parseHireDeepLink(link: string): string | null {
  ...
  if (u.protocol !== 'rudyos:') return null;
  // Both rudyos://hire?src= (host) and rudyos:hire?src= (path).
