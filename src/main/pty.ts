...
interface PtySession {
   ...
   *  file) and the idle handshake that gates boss's PTY nudge (never type into a
...
export class PtyManager {
  ...
  redraw(id: string): { ok: boolean; error?: string } {
    ...
    if (!s) return { ok: false, error: `no pty: ${id}` };
    try {
      // A same-size resize is a silent NO-OP: the kernel only delivers
      // SIGWINCH when the winsize actually CHANGES, so this "redraw" never
      // repainted anything — which is why a freshly attached terminal could
      // sit blank until the user resized something by hand. Jiggle one row
      // down and back so the TUI receives two real SIGWINCHes and repaints.
      const { cols, rows } = s.proc;
      s.proc.resize(cols, Math.max(1, rows - 1));
      setTimeout(() => {
        try {
          const cur = this.sessions.get(id);
          if (cur === s) s.proc.resize(cols, rows);
        } catch { /* pty died between the two — nothing to repaint */ }
      }, 30);
      return { ok: true };
    } catch (e) {
      return { ok: false, error: e instanceof Error ? e.message : String(e) };
