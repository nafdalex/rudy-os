...
import { Application, Container, Graphics, Ticker, Texture , ColorMatrixFilter } from 'pixi.js';
...
const ERRAND_THOUGHTS: Record<ErrandKind, readonly string[]> = {
  ...
  smoke:     ['the floor runs itself 🚬', 'boss break.', 'thinking big thoughts 🚬', 'a break is a deliverable']
};
...
export function OfficeFloor() {
  ...
  // One office, one theme: the TV-show reskins were removed with their picker.
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      await app.init({
        // Transparent: the letterboxing around the map takes the CARD's cream
        // instead of a black sea — the office floats on the app's paper.
        backgroundAlpha: 0,
        antialias: false,
        roundPixels: true,
      ...
      // A quieter colorway: the inherited tileset reads full-saturation
      // sitcom green; pulling saturation down folds the floor into the
      // shell's terminal grays without touching a single tile.
      const muted = new ColorMatrixFilter();
      muted.saturate(-0.3, false);
      world.filters = [muted];

      const mapRenderer = new TiledMapRenderer(resolveThemeMap(theme), tilesetTextures);
      world.addChild(mapRenderer.getContainer());
      // Colorway: tint only the ground tile layers. Furniture and the cast
      // are sibling containers, so they keep their own colors; the tint
      // multiplies down before the world's desaturation filter above.
      if (theme.palette.ground) {
        const layerRoot = mapRenderer.getContainer();
        const floorLayer = layerRoot.getChildByLabel('floor') as Container | null;
        const wallLayer = layerRoot.getChildByLabel('walls') as Container | null;
        if (floorLayer) floorLayer.tint = theme.palette.ground.floorTint;
        if (wallLayer) wallLayer.tint = theme.palette.ground.wallTint;
      }
      ...
      // selects Rudy (the boss) and opens the Command Center's TRIGGERS tab —
      ...
      calG.on('pointertap', (ev) => {
        ...
        const boss = st.agents.find((a) => a.isBoss);
        if (boss) st.select(boss.id);
      ...
      // Seat 0 is desk-ceo — "Rudy's room" — reserved for the boss agent.
      // All other workers claim seats from 1 onward.
      const BOSS_SEAT = 0;
      const claimSeat = (agent: Agent): number | null => {
        if (agent.isBoss) { seatClaims.add(BOSS_SEAT); return BOSS_SEAT; }
      ...
      /** Distance from the boss's avatar in px, or Infinity when he's absent. */
      const bossDistance = (px: number, py: number): number => {
        const boss = useStore.getState().agents.find((a) => a.isBoss);
        const grt = boss ? runtimes.get(boss.id) : undefined;
      ...
      const emitQuip = (id: string, rt: Runtime, spotIdx: number): void => {
        ...
        if (bossDistance(p.x, p.y) > 96 && Math.random() < 0.35) {
      ...
      const endBreak = (id: string, rt: Runtime): void => {
        ...
        if (agent?.isBoss) { rt.character.sitAtDesk(true); return; }
        const c = rt.character;
        if (!arrived) {
      ...
      const breakEligible = (agent: Agent, rt: Runtime): boolean => {
        if (agent.isBoss || rt.brk || rt.err || rt.run || rt.cupCarryHome) return false;
      ...
      const updateErrands = (dt: number): void => {
        ...
        const spot = ERRAND_SPOTS[idx];
        // Pick the performer. The CEO office's spots belong to Rudy alone —
        ...
        if (spot.bossOnly) {
          const boss = useStore.getState().agents.find((a) => a.isBoss);
          const grt = boss ? runtimes.get(boss.id) : undefined;
          if (!boss || !grt || grt.err || grt.brk
            || (boss.status !== 'idle' && boss.status !== 'success')
        ...
        c.walkToAndThen(spot.stand, () => {
          ...
          const finish = (): void => {
            const wasBoss = !!agent!.isBoss;
            releaseErrand(rt!);
            c.hideThought();
            if (wasBoss) c.sitAtDesk(true);  // the boss returns to his throne
            else c.startWandering();
          };
      ...
      // ─── The boss aura: performative excellence in Rudy's presence ──────
      // When the boss's avatar wanders close to a worker, the worker bursts
      // into suck-up mode — including REAL stats ("already shipped N tasks,
      // Rudy. raise?" with N from the actual ledger). What they say once
      ...
      const updateDeskLife = (dt: number): void => {
        for (const [id, rt] of runtimes) {
          ...
          if (rt.cupCarryHome && rt.character.isSittingAtDesk()) {
            ...
            const agent = agentById(id);
            if (agent && !agent.isBoss && (agent.status === 'idle' || agent.status === 'success')) {
              rt.character.startWandering();
            }
          }
      ...
      // Rudy and opens the Command Center's tasks tab.
      ...
      // The wall clock beside Rudy's window doubles as the quit entry:
      ...
      // ─── The NEEDS YOU board: tasks waiting on the HUMAN, first class ──────
      // Hangs on the studio's lower wall, right beside reception — questions
      // for the human wait where the human walks in. One lilac note per open
      // question; it pulses while anything waits, and clicking it opens the
      // NEEDS YOU sheet. (It used to float mid-floor at the old map's wall
      // coordinates, which read as a lost crate.)
      const askG = new Graphics();
      askG.eventMode = 'static';
      askG.cursor = 'pointer';
      askG.position.set(1 * tsB + 8, 9 * tsB - 6);
      askG.zIndex = 10 * tsB;
      ...
      // Rudy walks over and pins fresh cards; an assigned worker walks to
       ...
       *  boss for fresh pins / orphan cards. Returns undefined → instant redraw. */
      const actorFor = (assignee: string | undefined, preferBoss: boolean): string | undefined => {
        if (!preferBoss && assignee && runtimes.has(assignee)) return assignee;
        const boss = useStore.getState().agents.find((a) => a.isBoss);
        return boss && runtimes.has(boss.id) ? boss.id : undefined;
      };
      ...
      const applyState = (agent: Agent, rt: Runtime, force = false) => {
        ...
        const finishedWork = !force && !agent.isBoss
        ...
        switch (agent.status) {
          ...
          case 'thinking':
            ...
            break;
          case 'waiting':
            // Parked at the desk awaiting boss / another agent — not actively
          ...
          case 'success':
            ...
            if (agent.isBoss) { c.hideThought(); c.sitAtDesk(true); break; }
            c.startWandering();
            if (finishedWork) {
              c.cheer();
          ...
          default:
            c.setStatusGlyph('none');
            // The boss runs the floor from its desk; everyone else wanders when idle.
            if (agent.isBoss) { c.sitAtDesk(true); c.showThought(liveActivity(agent, 'running the floor')); }
      ...
      // Coalesce to one resize per frame, and paint a fresh frame IN the same
      // frame: without the immediate render, the grow/shrink jump showed one
      // stale frame stretched to the new box before the ticker caught up.
      let pendingResize: { width: number; height: number } | null = null;
      const resize = new ResizeObserver((entries) => {
        for (const e of entries) {
          const { width, height } = e.contentRect;
          if (width === 0 || height === 0) continue;
          const first = pendingResize === null;
          ...
          requestAnimationFrame(() => {
            ...
            camera.setViewSize(r.width, r.height);
            try { app.render(); } catch { /* mid-teardown */ }
          });
        }
      });
      resize.observe(host);
  ...
  return (
    <div
      ...
      style={{
        ...
        // Transparent like the canvas: the surrounding card provides the
        // ground, so no black flash and no black letterboxing.
