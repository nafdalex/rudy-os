...
`IntegrationTemplate`** in `src/shared/integrations.ts` (Theo's registry/broker spec —
...
no separate catalog. The **loopback broker** (Theo) makes the actual HTTP calls and is
...
Theo's `IntegrationAuthType` has no `oauth2`, and OAuth refresh is a v1 non-goal (spec
...
secret), which Theo's v1 schema does not model: `IntegrationAuthType` has no `oauth2`
...
- **Conformed to Theo's canonical schema.** The 7 first-wave entries are appended
...
- **Flagged gaps (god → Theo, enum widening rather than forking the type):**
...
- **Three `IntegrationTemplate` shapes still exist** across the tree (this file →
  conformed; Theo's `integrations.ts`; Yuki's renderer `registryClient.ts` mock). Full
  collapse to one needs Yuki's Settings UI to consume the shared templates — a
...
- **Out of scope (other agents):** the registry/broker runtime (Theo) and the Settings
  UI (Yuki). This deliverable is the conformed template data + this doc only.
