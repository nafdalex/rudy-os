...
import { acquireTerminal, applyTerminalTheme, attachTerminal, detachTerminal, reflowTerminal } from './terminalPool';
import {
...
export function PtyTerminalView({ ptyId, onStreamData, onUserPrompt, onToggleFullscreen, fullscreen, embedded }: PtyTerminalViewProps) {
  ...
  useEffect(() => {
    ...
    applyTerminalTheme(entry, THEMES[ptyThemeRef.current]);
    ...
    // Safety net for the empty-screen attach: if after settling the visible
    // buffer holds NO text while the pty is live, the TUI simply has not
    // painted for us yet — ask it to (redrawPty jiggles a real SIGWINCH).
    // Repeatable per attach, unlike the one-shot initial-redraw request.
    const emptyProbe = window.setTimeout(() => {
      try {
        const buf = entry.term.buffer.active;
        const rows = Math.min(entry.term.rows, 50);
        let hasText = false;
        for (let i = 0; i < rows; i++) {
          const line = buf.getLine(buf.viewportY + i);
          if (line && line.translateToString(true).trim()) { hasText = true; break; }
        }
        if (!hasText && !entry.exited) void window.cth.redrawPty(ptyId);
      } catch { /* buffer not ready — the next attach probes again */ }
    }, 800);
  ...
  useEffect(() => {
    applyTerminalTheme(acquireTerminal(ptyId, THEMES[ptyTheme], fontSizeRef.current), THEMES[ptyTheme]);
  }, [ptyTheme, ptyId]);
