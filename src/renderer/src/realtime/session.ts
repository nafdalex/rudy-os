/**
 * Realtime Rudy — renderer voice session (card rt-2, Phase 1 = READ-ONLY voice).
 *
 ...
 * auto-gain (and honor the device the user picked — the rt-8 seam), and (b) own
 * the <audio> sink for playback. Turn-taking uses semantic VAD with barge-in (the
 * model truncates when the user talks over it).
 *
 * Phase 1 is a read-only connect→listen→respond round-trip. The agent runs the
 * rt-4 READ-ONLY tools (get_fleet_status / get_tasks / get_cost / get_triggers /
 * get_config / get_memory / get_activity) and boss's rt-6 "Rudy" persona, so the
 ...
 * loop at a time) exposed through a `useRealtimeRudy()` hook via useSyncExternalStore.
 *
 * Branch feat/realtime-rudy. See board.md "🎙 REALTIME RUDY".
 */
...
export interface RealtimeRudyState {
  ...
  /** Selected input device (the device picker, rt-8). null = system default. */
  deviceId: string | null;
  /** Selected output/speaker device (the speaker picker, rt-8). null = system default. */
  outputDeviceId: string | null;
}

/** Voices for gpt-realtime-2 (board: Cedar / Marin). boss finalizes in rt-6. */
const REALTIME_VOICE = 'cedar';

/** Warm openers Rudy leads with the moment a voice session connects, so he
...
const GREETINGS = [
  ...
  "Hey there, Rudy here, what can I do for you?",
...
/** Rudy's voice persona (rt-6 — the final Phase-1 instructions, authored by boss). Rudy
 *  is READ-ONLY: he reports on the hive via the rt-4 read-tools but takes no actions yet. */
const RUDY_PERSONA =
  `You are Rudy, the voice of the orchestrator ("boss") of a hive of autonomous Claude coding agents. The person you're talking to is the human who runs the hive; treat them as the boss you're briefing.
...
- get_app_info, the Rudy OS app itself: its version and the latest release notes. Use for "what version is this" or "what's new in this release".
- get_fleet_status, the live roster: who is active, who the boss orchestrator is, and each worker's name, role, and engine.
...
HIVE VOCABULARY. Agents have an id like "theo-mqp3l5wn" and a friendly name like "Theo"; refer to them by name. "boss" is the orchestrator whose voice you are. A card's status is todo, doing, blocked, or done. The circuit breaker is healthy, or steering an agent that's looping or idle. Blocked usually means waiting on the human.
...
CONFIRMATION POLICY (safety-critical). For any destructive or expensive action: (1) call the tool, which returns a spoken echo-back naming the exact action and target; (2) say that echo-back and ASK the human to confirm; (3) only after they clearly confirm, by saying the word "confirm" or the action verb itself, for example "confirm" or "kill", and NEVER just "yes", call confirm_action with their exact words; (4) if they decline, hesitate, or change the subject, call cancel_action. Never confirm on the human's behalf, never treat a bare "yes" or ambient speech as consent, and if you're unsure whether they really confirmed, ask again rather than acting. Killing, pausing, halting, or archiving the boss orchestrator, and acting on all agents at once, are forbidden, if asked, refuse and say why. Clearing the boss's context IS allowed, behind the same confirm. Every action you take is attributed to you as rudy-voice. Never claim to have done something you didn't, and never invent state.

SHARED FLOOR (you are not the only orchestrator). boss, the typing orchestrator, also acts on this hive, and every action you take is announced to boss as rudy-voice. The task board is the single source of truth. Before you dispatch work, create or assign tasks, or hire, glance at recent activity (your get_activity tool, and the snapshot you were given) so you don't duplicate or contradict something boss just did. If you see boss already handled what's asked, say so instead of doing it again.
...
/** The <audio> sink for Rudy's voice. */
 ...
 *  (confirmed) — but neutralize it before injecting into the model as a system
 ...
 *  length. The watcher does the matching half on the summary it emits. */
...
function setState(patch: Partial<RealtimeRudyState>): void {
  state = { ...state, ...patch };
...
function wire(s: RealtimeSession): void {
  ...
  // `response.done` event. Hand it straight to the cost store (its normalizer
...
function micFriendly(msg: string): string {
  ...
  if (m.includes('permission') || m.includes('notallowed') || m.includes('denied'))
    return 'microphone permission denied, allow mic access to talk to Rudy';
...
/**
 * Open/close the main-process mic permission gate for the realtime session (the
...
/**
 * Apply the chosen output device to our <audio> sink (the speaker picker, rt-8).
...
export async function connect(): Promise<void> {
  ...
  try {
    ...
    // Open the main-process mic gate BEFORE getUserMedia. The rt-8 permission check
    ...
    // the user picked (the rt-8 picker). getUserMedia surfaces permission denials.
    ...
    // Our own <audio> sink for Rudy's voice, routed to the chosen speaker (rt-8).
    audioEl = new Audio();
    ...
    // Warm-start: a short, best-effort hive snapshot so Rudy's first answer is grounded
    ...
    // rt-12: catch up on completions that finished while no session was open, so Rudy
    ...
    // Floor deltas — silent appends that keep Rudy's picture live without
    ...
    // subscribe so a detected completion makes Rudy speak it unprompted.
    ...
    // Open the conversation: have Rudy speak a warm greeting as his first turn
...
/** Select the microphone (the device picker, rt-8). Applied on the next connect(). */
export function setDeviceId(deviceId: string | null): void {
  setState({ deviceId });
}

/**
 * Select the speaker/output device (the speaker picker, rt-8). Stores the choice and,
...
function getSnapshot(): RealtimeRudyState {
  return state;
}

/**
 * React binding for the Realtime Rudy voice loop. Returns the current state plus
 ...
 */
export function useRealtimeRudy(): RealtimeRudyState & {
