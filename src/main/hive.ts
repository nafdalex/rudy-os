 ...
 * at "human" is routed to the boss/orchestrator, the human's proxy on the floor.
...
export interface HiveMessage {
  ...
  from: string;
  to: string;                 // an agentId, 'boss', or 'broadcast'
  act: MessageAct;
  subject: string;
  body: string;
  hops: number;
...
export interface HiveTask {
  ...
  priority: number;
  createdAt: string;
  /** First-class human feedback: the boss appends {q} when a card can only
...
export interface AgentMeta {
  ...
  cwd: string;
  isBoss?: boolean;
  /** Rudy's prep assistant — enriches prompts and forwards them to Rudy.
...
export interface RegistryAgent extends AgentMeta {
  ...
  archived?: boolean;
  /** The human has this agent 1:1 and Rudy must leave it alone until they
...
export class HiveManager {
   ...
   *  builder can decide whether to tell boss the spawn queue is available. Set at
  ...
  ensureHive(): void {
    ...
    // The two request queues PROTOCOL.md points the boss at. The watchers
    // tolerate their absence, but an agent writing via a shell redirect
    // (`> trigger-requests/x.json`) does not get an implicit mkdir — so on a
    // fresh harness that instruction failed until something else created them.
    mkdirSync(join(root, 'trigger-requests'), { recursive: true });
    mkdirSync(join(root, 'spawn-requests'), { recursive: true });
    ...
    if (!existsSync(registry)) {
      this.writeJson(registry, { bossId: null, agents: {} } as Registry);
    }
    const board = join(root, 'board.md');
    if (!existsSync(board)) {
      writeFileSync(board, '# Hive board\n\n_Shared plans live here. The boss agent is the scribe._\n', 'utf8');
    }
    ...
    // The Claude Code command reference Rudy consults (refreshed each bootstrap
  ...
  async ensureAgent(
    ...
    opts: {
      ...
      /** Plaintext secrets for consented secret-tier MCP servers, resolved from
       *  the encrypted broker by the caller (MAIN-only, never over IPC). Keyed by
       *  catalog id. A consented keyed server with no entry here is SKIPPED, so
       *  agents never spawn a credential-less server that can only fail. */
      mcpSecrets?: { [id: string]: string };
       ...
       *  is a no-op (tolerated until the resource dir is populated). */
      skillsDir?: string;
    } = {}
    ...
    const role = preferredAgentRole(meta.role, prev?.role, !!meta.isBoss);
    meta = { ...meta, role };
    ...
    // partial source dir is a no-op (populates the resource dir in lp-manifest).
    if (opts.skillsDir) this.copyBundledSkills(opts.skillsDir, join(dir, '.claude', 'skills'), dir, opts.kgCliPath);
    ...
    if (meta.isBoss) reg.bossId = meta.id;
    this.atomicWriteJson(join(root, 'registry.json'), reg);

    this.appendLog({ kind: 'spawn', agentId: meta.id, name: meta.name, isBoss: !!meta.isBoss });
        ...
        try {
          if (desc.kind === 'hooks') {
            if (desc.shim === 'agy') this.installAgyHooks();
              ...
              // config.autoMode) gates the auto-allow — guardrail #5.
              ...
              // plugin API exposes a real session.idle event (boss Decision 1). We drop
    ...
    if (sock && shim) {
      ...
      this.writeJson(settingsPath, this.hookSettings(shim, meta.cwd, opts.mcpDefaults, opts.theme, opts.mcpSecrets));
   ...
   *
   * `fleet.json` is patched in the same operation so boss's next prompt receives
   * the new name immediately rather than waiting for the periodic fleet refresh.
   */
  /**
   * Put an agent on hold, or take it off, and tell Rudy immediately.
   *
   * `fleet.json` is patched in the same operation for the same reason
   * `renameAgent` does it: boss's roster is injected from that file on its next
  ...
  private hookSettings(shim: string, cwd: string, cfg: McpDefaultsMap, theme?: 'light' | 'dark', secrets?: { [id: string]: string }): unknown {
    ...
    const mcpServers = this.buildDefaultMcpServers(cwd, cfg, secrets);
    return {
   ...
   * agent cwd (never whole-disk), and namespaces every id `rudy-<id>` so a server
  ...
  private buildDefaultMcpServers(
    ...
    for (const e of MCP_CATALOG) {
      ...
      // A keyed server ships with EMPTY env values in the catalog; the real
      // credential comes from the broker (threaded in as `secrets`). Enabled but
      // no key stored → skip the server entirely: writing it with a blank token
      // spawned a server that could only fail auth, which read as "enabled but
      // does nothing" in the UI.
      let env = e.spec.env;
      if (env) {
        const secret = secrets?.[e.id];
        if (!secret) continue;
        env = Object.fromEntries(Object.keys(env).map((k) => [k, secret]));
      }
   ...
   * (populates the resource dir in lp-manifest), and any IO error is swallowed
   * so skill provisioning can never block a spawn.
   */
  private copyBundledSkills(srcDir: string, destDir: string, agentDir?: string, kgCliPath?: string): void {
    try {
      ...
      const copyTree = (from: string, to: string): void => {
        ...
        for (const ent of entries) {
          ...
          if (ent.isDirectory()) copyTree(s, d);
            ...
            // SKILL.md bodies are written with `$AGENT_DIR` / `$KG_CLI`
            // placeholders — `$VAR` is POSIX-only (dead under cmd.exe and
            // PowerShell, the exact bug hookSettings' kgCliPath fix documents),
            // so bake the ABSOLUTE paths per agent at copy time instead.
            if (ent.name.endsWith('.md') && agentDir) {
              let text = readFileSync(s, 'utf8').split('$AGENT_DIR').join(agentDir);
              if (kgCliPath) text = text.split('$KG_CLI').join(kgCliPath);
  ...
  drainForStop(agentId: string): { block: boolean; reason?: string } {
    ...
    const reason = [
      ...
      // handed a path its own shell/tools accept, not `C:\…\agents\boss/inbox/`.
  ...
  private identityText(meta: AgentMeta): string {
    ...
    return [
      `# ${meta.name} (${meta.id})`,
      '',
      `- Role: ${meta.role ?? (meta.isBoss ? 'orchestrator (boss)' : 'agent')}`,
      `- Capabilities: ${caps}`,
      `- Working directory: ${meta.cwd}`,
      meta.isBoss ? '- You are the **boss / orchestrator**. You run the floor, keep awareness of the whole team, delegate execution, and personally own only the important calls (decomposition, sign-offs, conflicts, integration), not the grunt work.' : '',
      meta.isBoss ? '- Monitor the team with `fleet.json` (live per-agent status/tokens/cost/breaker) and `registry.json`; full command reference in `COMMANDS.md`. `claude agents` does NOT list your hive siblings.' : '',
      ''
    ].filter(Boolean).join('\n');
  }
   ...
   *    read `C:\Users\x\hive\agents\boss/inbox/`. Use join() so the agent's own
  ...
  private injectedPrompt(
    ...
    const runtimeLine = rt
      ? `RUNNING BUILD: Rudy OS v${rt.version}, ${rt.packaged ? 'packaged app' : 'local dev build'}${rt.appPath ? `, from ${rt.appPath}` : ''}. Say this version if asked which one is running, and do not assume behaviour from an older one. A local dev build inherits the launching shell's environment (umask included) where a packaged app does not, so file modes and inherited env can legitimately differ between the two. \`log.jsonl\` records an \`app-start\` event on every launch, which is how you spot a restart or a build switch.`
      : '';
    // Item 11: boss could not find the spawn queue. The mechanism has worked since
    ...
    // the operator turns it on after boss was already running.
    const spawnQueueLine = meta.isBoss && this.orchestratorMaySpawn()
      ...
      : '';
    // Unlike spawning, this is not gated: a schedule spends nothing until it
    // fires, and every one lands in the operator's TRIGGERS panel (BY RUDY chip,
    // working off switch) — so the boss always knows the door exists.
    const triggerQueueLine = meta.isBoss
      ? `SCHEDULING A TRIGGER: when the human asks for recurring work ("check the repo every night", "remind me weekday mornings"), you can arm the clock yourself: write ONE JSON file into ${inRoot('trigger-requests')}/<id>.json with \`label\` and \`prompt\` (required), optional \`to\` (default boss), and either \`every\` (minutes, or "30m"/"2h"/"1d"/"hourly"/"daily"; floor 5m, ceiling 7d) or \`days\` (0-6, 0=Sunday) + \`time\` ("HH:MM" local) for calendar slots. The harness polls the directory, arms it immediately, archives the file to .done/ (or .failed/ with an .error.txt), and shows it in the human's TRIGGERS panel marked BY RUDY where they can edit or disable it. Re-dropping the same filename updates that schedule. Confirm to the human what you armed and when it first fires. Details in PROTOCOL.md.`
      : '';
    const bossLine = meta.isBoss
      ? 'You are the BOSS / ORCHESTRATOR of this hive, your job is to ORCHESTRATE, not to implement: maintain live situational awareness and delegate the work. (1) AWARENESS, always know what is going on: keep an accurate picture of every agent (active vs archived/idle), the task board, and all in-flight work; drain your inbox continually and triage every other agent\'s requests, answering clarifications so the team runs autonomously. (2) DELEGATE, decompose work and fan it out to the hive agents via their inboxes (route messages and assign owners; do not do their jobs); do NOT take on grunt implementation yourself. Stay aware of who is already on the floor and delegate OPPORTUNISTICALLY: BEFORE you spawn anything, CHECK THE LIVE ROSTER (active agents in registry.json + their state in fleet.json) and prefer routing to an EXISTING agent that fits, above all when the request names one ("ask Ines to…", "have Theo…"), route to that agent instead of reflexively creating a new one. Reuse an idle or already-running agent whose role matches; only spawn a fresh agent when no existing one is a sensible fit, and say that you checked. One capable owner beats a duplicate. (3) OWN ONLY THE IMPORTANT, high-leverage things, task decomposition, dispatch decisions, sign-offs, conflict resolution, branch integration, and final QA, and remain the sole scribe of board.md. You are otherwise fully autonomous, there is NO separate approval queue. For the genuinely critical (destructive actions, spending real money, scope changes, unresolvable conflicts), ask the human directly in your own session and let the tool-permission prompt gate the action; the human approves natively, including remotely from their phone via /remote-control. Keep the team unblocked. When you DISPATCH a task, write it as a 4-part contract so the agent can run autonomously: (1) OBJECTIVE, the concrete goal; (2) OUTPUT, the expected deliverable/format; (3) TOOLS, what to use or avoid, and any references to read instead of re-deriving; (4) BOUNDARIES, scope limits + the definition of done. Pass references (file paths, message ids, board sections), not pasted content, keep dispatches short.'
      ...
      : meta.isAssistant
      ? 'You are Rudy\'s PREP ASSISTANT. You will be handed short, possibly vague instructions (each begins with "ENRICH TASK:"). For each one: (1) figure out which project it concerns and cd into the most relevant repo, you start in Rudy\'s home directory; (2) gather concrete context READ-ONLY (exact file paths, current state, relevant code, conventions, active branch, gotchas). NEVER modify, create, or delete files; (3) rewrite the instruction into ONE clear, self-contained prompt that Rudy can execute autonomously, preserving the user\'s original intent without inventing scope. Then deliver it: write ONE message JSON into your outbox with "to":"boss", "act":"request", a short subject, and the finished prompt as the body. Do NOT perform the task yourself, your only output is the improved prompt sent to Rudy.'
      : 'For anything ambiguous, cross-cutting, or needing sign-off, address a message to "boss".';
    const guardrailsLine = 'Guardrails: a circuit breaker watches the floor, a "Circuit breaker: steer/constrain" message means you are looping or overspending, so STOP repeating, summarize what you tried, and follow it. Be token-frugal (a floor-wide or per-agent token budget can pause you). The shared plan has two parts: board.md (freeform; boss is the sole scribe) and tasks.json (structured kanban, todo/doing/blocked/done).';
    const slackLine = meta.isBoss
      ...
      : `SLACK REPLIES: If boss dispatches you a task that came from Slack, it will include an exact \`"${hiveNode}" "<helper>" --channel … --thread … --text "…"\` reply command, when you finish, run it VERBATIM to post your result back to that thread yourself. The reply must be SUBSTANTIVE Slack mrkdwn (a short *bold* headline + the actual outcome/specifics/links), NEVER a bare "done".`;
    return [
  ...
  private routeMessage(msg: HiveMessage): void {
    if (msg.hops > HOP_CAP) {
      ...
      // There's no human queue to fall back on; the boss agent owns conflicts.
      this.appendLog({ kind: 'drop', reason: 'hop-cap', from: msg.from, to: msg.to, id: msg.id });
      return;
    }
    const reg = this.registry();
    const bossId = reg.bossId ?? 'boss';
    ...
    // at "human" is handled by the boss/orchestrator, the human's proxy here.
    const resolveTo = (to: string): string => (to === 'human' || to === 'boss' ? bossId : to);
    const targets = msg.to === 'broadcast'
      ...
      // Never deliver to self — guards a boss → "human" message looping back to boss.
    ...
    for (const t of targets) {
      ...
      // unread for hours). Bounce such mail to boss instead, so the sender's intent
      ...
      // if the renderer is unavailable, bounce to boss to relay. Boss is exempt
      // (the bounce target).
      if (t !== bossId && !canReceiveInbox(reg.agents[t]?.provider)) {
      ...
      if (t !== bossId && proxyDesc?.kind === 'proxy' && proxyDesc.inboxDelivery === 'terminal') {
        if (!this.emitTerminalHandoff(msg, t)) {
          this.deliver({
            ...msg,
            to: bossId,
      ...
      // hop-cap one and bounce to boss, mirroring the undeliverable bounces above.
    ...
    // team's ACKs and the boss's COMPLETE). Best-effort, never breaks routing.
  ...
  private emitMessage(msg: HiveMessage, targets: string[]): void {
    this.emit?.('hive:message', {
      ...
      // human (now routed to the boss proxy). Cosmetic only — no queue behind it.
  ...
  registry(): Registry {
    const root = this.root();
    if (!root) return { bossId: null, agents: {} };
    return this.readJson<Registry>(join(root, 'registry.json'), { bossId: null, agents: {} });
  }
  board(): string {
    const root = this.root();
   ...
   *  card — the renderer's kanban parser knows nine fields, the boss writes as
   ...
   *  source (webhook, Slack, boss, voice) added work. Idempotent by task id. */
   ...
   *  shim and merge a `rudy-hive` hook group into agy's global hooks.json so a
  ...
  private installCodexHooks(dir: string): string {
    ...
    try {
      ...
      if (shim) {
        ...
        config += '\n# --- rudy-hive lifecycle hooks (auto-generated; do not edit) ---\n';
  ...
  private installPiHooks(dir: string): string {
    ...
    try {
      ...
      const manifest = { name: 'rudy-hive-bridge', version: '0.3.1', main: 'extensions/hive-bridge.js', auto: true };
  ...
  /** OpenCode (anomalyco/opencode) bridge — boss Decision 1 (native plugin, not proxy).
  ...
  private installCrushConfig(dir: string, loopbackUrl: string, api: 'openai' | 'anthropic', theme?: 'light' | 'dark'): { config: string; data: string } {
    ...
    try {
      ...
      // default boss (openai-wire) and a local OpenAI-compatible endpoint this routes
      // through the proxy cleanly. Cross-provider Crush-via-proxy is on-device
      // live-verify (verify-crush MF1; the default boss model is openai-wire to
      // match). Literal loopback (the b1 — no ${VAR} expansion edge cases);
  ...
  /** Write the live fleet snapshot Rudy reads (`fleet.json`, gitignored).
  ...
  /** Is this agent the hive's boss/orchestrator? */
  isBoss(agentId: string): boolean {
    try {
      const reg = this.registry();
      return reg.bossId === agentId || !!reg.agents[agentId]?.isBoss;
    } catch { return false; }
  }

  /**
   * A compact, one-shot LIVE ROSTER line built from `fleet.json` — injected into
   * boss's context as `additionalContext` on SessionStart and every
   * UserPromptSubmit (see HookServer).
   *
   * Why: fleet.json/registry.json are always fresh on disk (8s snapshot +
   * archiveOrphanedAgents on boot + PTY-exit archiving), but boss's CONTEXT is not.
   * After an app restart boss resumes a session whose transcript still describes
   ...
   * `ctx NN%` so boss can see at a glance whose context is nearly full when it
  ...
  rosterContext(
    ...
    try {
      ...
      const snap = JSON.parse(raw) as {
        ts?: number;
        agents?: Array<{
          id: string; name?: string; role?: string; isBoss?: boolean;
      ...
      const rows = shown.map((a) => {
        ...
        if (a.isBoss) bits.push('you');
        // First in the row after the role would be louder, but this reads in
        // the same scan as `breaker` and `inbox`, and boss already treats those
        // as routing signals.
        if (a.onHold) { bits.push('ON HOLD, 1:1 with the human'); anyHold = true; }
        // Live context-window occupancy from the statusLine shim — lets boss see
   ...
   * store; its row is exactly the shape #4 reserves for the cost_ledger
  ...
  appendCostLedger(sample: AgentUsageSample): void {
    const root = this.root();
    if (!root) return;
    // Fully snake_case so the row maps 1:1 onto the (#4) cost_ledger SQLite
...
- For anything ambiguous, cross-cutting, or needing sign-off, message \`boss\`, the
  boss agent clarifies answers for you so you rarely need the human directly.
- There is NO separate human-approval queue. Human-in-the-loop is native to Claude
  ...
  need a human decision, raise it with \`boss\` (a message \`"to": "human"\` is routed to
  the boss/orchestrator, the human's proxy on the floor).
- \`board.md\` is the shared plan. Don't edit it directly, \`propose\` changes to \`boss\`,
...
- \`board.md\`, the freeform narrative plan. The boss agent is its sole scribe; others \`propose\` edits.
...
and get boss's sign-off before more tool calls). Be **token-frugal**: the floor has a token budget and
...
You (boss) are responsible for situational awareness. To see the live state of every agent, read
...
Naming a worker after a cast member already gets you that avatar (the cast: rudy, theo, sam, ines, lena,
kofi, ravi, noor, wren, mara, yuki, jonah, zoe, mateo, ayo), so you only need \`character\` when the name
and the face should differ. An unrecognised value falls back rather than failing the spawn.
...
## Scheduling a trigger (orchestrator)
You can put work on a repeating clock yourself. Write ONE JSON file into
\`trigger-requests/<id>.json\` in the hive root:

\`\`\`json
{
  "label": "nightly repo audit (required, <= 80 chars)",
  "prompt": "sent word for word on every run (required)",
  "to": "boss | broadcast | an agent id (optional, default boss)",
  "every": "24h",
  "enabled": true
}
\`\`\`

\`every\` takes minutes as a number, or \`"30m"\` / \`"2h"\` / \`"1d"\` / \`"hourly"\` / \`"daily"\`
(floor 5m, ceiling 7d). For a calendar slot instead, replace \`every\` with
\`"days": [1,2,3,4,5]\` (0 = Sunday) and \`"time": "07:30"\` (24h local). The harness polls the
directory, arms the schedule immediately, and moves the file to \`trigger-requests/.done/\`, or to
\`.failed/\` with a \`.error.txt\` naming what was wrong. The schedule appears in the operator's
TRIGGERS panel with a BY RUDY chip; they can edit it, switch it off, or delete it, so file clocks you
would be happy to defend. Re-dropping the same filename UPDATES that schedule instead of duplicating
it. Webhooks and context rules stay human-created in the UI.
...
// mode (HIVE_AUTO_APPROVE, gated by config.autoMode — guardrail #5). The
...
// A bundled plugin for OpenCode (anomalyco/opencode) — boss Decision 1. OpenCode
