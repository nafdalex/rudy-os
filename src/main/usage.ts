/**
 * Usage telemetry seam (Lane A #6.6 — Seam 1, the LOCKED contract with #7).
 *
 ...
 *
 * Two invariants every consumer must honor (the 7A.1 spike findings):
 ...
 * "transcript-poll" backend that will evolve, then replace with the
 * native-OTel collector. At integration we drop in the module; breaker.ts and
 * the ledger are untouched. The stub's `usd` is the transcript fallback estimate
 * (and inherits the known Sonnet-hardcoded pricing limitation, fixed
 * in exactly one place — his provider); it is NOT recomputed downstream.
 */
import { readAgentUsage } from './transcript';

/** One cumulative usage snapshot for an agent. The identical row the provider
 *  emits, this lane persists to cost-ledger.jsonl, and #4 stores
 ...
 *  normalized form the OTel ingest emits. */
 ...
 * AgentUsageSample. Stands in for the provider until Lane C lands; the
...
export class StubUsageProvider implements UsageProvider {
  ...
  getAgentUsage(agentId: string): AgentUsageSample | null {
    ...
    return {
      ...
      usd: u.estimatedCostUsd // interim fallback estimate; The provider supplies Claude-precomputed usd
    };
  }
}
