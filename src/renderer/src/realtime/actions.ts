/**
 * Realtime Rudy — voice ACTION tools (card rt-5, Phase 2).
 *
 * The write-side function-tools that turn voice-Rudy into an orchestrator: ping
 ...
 * rule, the hard allowlist (kill-boss / mass-ops forbidden), and the rudy-voice
...
export function realtimeActionTools(): ReturnType<typeof tool>[] {
  return [
    ...
    tool({
      name: 'ping_agent',
      description:
        'Send a short message to one agent (a nudge or note). Soft action, runs immediately, no confirm. Use for "tell Ravi X" or "check in with Theo".',
      parameters: {
        type: 'object',
        properties: {
    ...
    tool({
      name: 'dispatch_agent',
      description:
        'Give an agent a task as a structured 4-part work order (objective, context, constraints, done-when) delivered to their inbox. Soft action, runs immediately. Use for "have Theo build X" or "ask Ravi to investigate Y".',
      parameters: {
        type: 'object',
        properties: {
    ...
    tool({
      name: 'steer_agent',
      description:
        'Inject live guidance into a running agent to redirect it without stopping it. Soft action, runs immediately. This is the priority verb: "tell Theo to focus on the bug first", "steer Ravi away from that approach".',
      parameters: {
        type: 'object',
        properties: {
    ...
    tool({
      name: 'kill_agent',
      description:
        'Terminate a running agent (closes its terminal, archives it). DESTRUCTIVE, does NOT run immediately; returns an echo-back and asks for verbal confirmation. After the user confirms, call confirm_action. Killing the boss orchestrator or all agents at once is forbidden.',
      parameters: {
        type: 'object',
    ...
    tool({
      name: 'gate_tool',
      description:
        'Block (gate) or unblock one named tool for one agent, e.g. gate Bash for Theo. Soft action, runs immediately.',
      parameters: {
        type: 'object',
        properties: {
          agentId: { type: 'string', description: 'Agent name or id.' },
    ...
    tool({
      ...
      description:
        "Queue a context clear (/clear) for one agent, wipes its working memory of the current conversation; delivery waits until the agent is idle. DESTRUCTIVE, returns an echo-back and asks for verbal confirmation ('clear' or 'confirm'). After the user confirms, call confirm_action. Allowed on the boss orchestrator too (it can resume its session).",
      parameters: {
        type: 'object',
    ...
    tool({
      ...
      parameters: {
        ...
        properties: {
          ...
          to: { type: 'string', description: 'Target agent name or id. Default: the boss orchestrator.' }
        },
