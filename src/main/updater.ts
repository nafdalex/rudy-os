...
export function pickDownloadAsset(
  ...
  // electron-builder.yml builds mac as `universal`, but process.arch reports
  // arm64/x64 — accept either, or every mac download 404'd on the shipped name.
  const want = platform === 'darwin' ? new RegExp(`-mac-(?:${arch}|universal)\\.dmg$`)
    ...
    const req = httpsRequest(
      {
        ...
        method: 'GET',
        headers: { 'User-Agent': 'rudy-os-updater', Accept: 'application/vnd.github+json' },
        timeout: 10_000
      },
      (res) => {
        let body = '';
...
const SIMULATED_NOTES = `# Rudy OS v9.9.9

**A local hive of Claude Code, Antigravity, Codex, Grok & Copilot agents that run themselves**, 
messaging, routing, and remembering, coordinated by your clone, Rudy, who you talk to.
