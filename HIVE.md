...
> How Rudy OS turns a room full of independent `claude`
...
3. **God-mode autonomy, native HITL.** A privileged **god agent** (lives in
   Rudy's room) adjudicates cross-agent traffic. Routine requests
...
A fixed, always-on agent seated at `desk-ceo` (Rudy's room), `character:
rudy`, flagged `isGod`. It is an ordinary `claude` process — the *intelligence*
...
- **Phase 2 — God mode** ✅: the god agent auto-spawns into Rudy's room
