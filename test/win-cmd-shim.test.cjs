...
const HOSTILE_PROMPT = [
  'You are "Rudy" (boss-1), an autonomous agent in a collaborating hive.',
  'Your private workspace is C:\\Users\\Tester\\.rudy\\hive\\agents\\boss-1.',
  '',
  'HIVE PROTOCOL — follow it every task:',
  '1. Read EVERY file in C:\\Users\\Tester\\.rudy\\hive\\agents\\boss-1\\inbox (messages',
...
test('cmd.exe: the same prompt is destroyed — which is the bug', () => {
  ...
  const atName = inner.indexOf('Rudy');
  assert.ok(atName > 0);
...
test('the hive prompt survives a direct-executable shim, where cmd.exe destroys it', () => {
  const prompt = 'You are "Rudy" (boss-1), an autonomous agent.\n\nHIVE PROTOCOL\n- inbox\n- outbox';
