...
test('Claude alone receives a remote-control slash command', () => {
  assert.equal(remoteControlCommandForProvider('claude', 'Rudy'), '/remote-control Rudy');
  assert.equal(remoteControlCommandForProvider('codex', 'Theo'), null);
  assert.equal(remoteControlCommandForProvider('grok', 'Grok'), null);
  assert.equal(remoteControlCommandForProvider('kimi', 'Ines'), null);
});
