...
/** An id seen in the message log that is no longer on the live roster — an
 *  archived or torn-down worker. Shown as a ghost instead of silently dropped:
 *  dropping it deleted real conversations from the picture, which is why the
 *  graph so often collapsed to just "human and Rudy". */
export interface GoneNode {
  kind: 'gone';
  id: string;
  label: string;
  degree: number;
}
export type GraphNode = AgentNode | TopicNode | PseudoNode | GoneNode;
...
export interface BuildOpts {
  ...
  maxTopics?: number;
  /** Resolve a gone id to a display name (archived roster lookup). */
...
export function buildGraph(
  ...
  const resolve = (ep?: string): string | null => {
    ...
    goneUsed.add(ep); // keep it: a gone worker's conversations still happened
    return ep;
  };
  ...
  for (let i = 0; i < log.length; i++) {
    ...
    // degree sizes agent + gone nodes (pseudo nodes stay fixed-size)
    if (byId.has(from) || goneUsed.has(from)) degree.set(from, (degree.get(from) ?? 0) + 1);
    if (byId.has(to) || goneUsed.has(to)) degree.set(to, (degree.get(to) ?? 0) + 1);
  }

  const nodes: GraphNode[] = [];
  const edges: GraphEdge[] = [];

  // agent nodes — only those that actually appear, plus boss, to avoid lone dots.
  ...
  for (const id of goneUsed) {
    nodes.push({ kind: 'gone', id, label: opts.nameFor?.(id) ?? id, degree: degree.get(id) ?? 0 });
  }
