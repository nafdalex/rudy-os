...
export interface TerminalEntry {
  ...
  ptyId: string;
  /** Per-terminal noise folded into the theme (see saltTheme) so no two
   *  terminals ever share a WebGL glyph atlas. Fixed at creation. */
  atlasSalt: number;
  term: Terminal;
  fit: FitAddon;
  ...
  generation: number;
  /** Last time the WebGL glyph atlas was preventively cleared (see the write
   * path). 0 = never. */
  atlasClearedAt: number;
  webgl?: WebglAddon;
}

const pool = new Map<string, TerminalEntry>();

/** Preventively rebuild an entry's WebGL glyph atlas, at most once per
 *  `minGapMs`, scheduled on a frame boundary (never inside a write callback).
 *  This is the ONLY reliable cure for the addon's silent atlas corruption —
 *  the terminal goes blank or garbled with NO event fired: the context is
 *  alive, the window focused, the buffer intact. */
function maybeHealAtlas(entry: TerminalEntry, minGapMs: number): void {
  const now = Date.now();
  if (!entry.webgl || !entry.opened || now - entry.atlasClearedAt < minGapMs) return;
  entry.atlasClearedAt = now;
  requestAnimationFrame(() => {
    if (!entry.webgl || !entry.host.isConnected) return;
    try {
      entry.term.clearTextureAtlas?.();
      entry.term.refresh(0, Math.max(0, entry.term.rows - 1));
    } catch { /* renderer settling */ }
  });
}

// An IDLE terminal gets no writes, so the per-write heal above never runs for
// it — which is exactly the boss terminal sitting blank while Rudy waits.
// Sweep every attached terminal on a slow clock; each heal is a lazy
// re-raster of only the glyphs actually on screen. The sweep also catches a
// renderer whose canvas is stuck at 0×0 inside a sized host (leased before
// layout by an older code path): that one needs a fresh lease, not a heal.
setInterval(() => {
  for (const entry of pool.values()) {
    if (!entry.host.isConnected) continue;
    if (entry.webgl && entry.host.clientWidth > 0) {
      const dead = Array.from(entry.host.querySelectorAll('canvas'))
        .some((c) => c.width === 0 || c.height === 0);
      if (dead) {
        releaseWebglRenderer(entry);
        leaseWebglRenderer(entry);
        maybeHealAtlas(entry, 0);
        continue;
      }
    }
    maybeHealAtlas(entry, 14_000);
  }
}, 15_000);
...
let atlasSaltCounter = 0;

/** Fold two nibbles of per-terminal noise into palette slots nobody can see
 *  (the low blue bits of brightBlack and brightWhite).
 *
 *  WHY: xterm's WebGL addon SHARES one glyph atlas between terminals whose
 *  options and colors are byte-identical (CharAtlasCache.configEquals). Two
 *  visible terminals then drive one atlas from two GL contexts, and any
 *  clearTextureAtlas heal on one leaves the other drawing from stale glyph
 *  coordinates — the reported "text turns to garbage when two Claude Code
 *  panes are open". A unique color config per terminal means a PRIVATE atlas
 *  per terminal, which ends the cross-talk at the source. */
function saltTheme(theme: ThemeMap | undefined, salt: number): ThemeMap | undefined {
  if (!theme) return theme;
  const out: ThemeMap = { ...theme };
  const tweak = (key: string, nibble: number): void => {
    const hex = out[key];
    if (!/^#[0-9a-fA-F]{6}$/.test(hex ?? '')) return;
    const n = parseInt((hex as string).slice(1), 16);
    out[key] = `#${(((n & 0xfffff0) | (nibble & 0x0f)) >>> 0).toString(16).padStart(6, '0')}`;
  };
  tweak('brightBlack', salt & 0x0f);
  tweak('brightWhite', (salt >> 4) & 0x0f);
  return out;
}

/** The one write path for a terminal's theme: always routes through the
 *  entry's atlas salt, so a view re-applying a shared THEMES object cannot
 *  silently reunify two terminals onto one shared atlas. */
export function applyTerminalTheme(entry: TerminalEntry, theme: ThemeMap): void {
  entry.term.options.theme = saltTheme(theme, entry.atlasSalt);
}
...
export function acquireTerminal(ptyId: string, theme?: ThemeMap, fontSize = 14): TerminalEntry {
  ...
  const atlasSalt = (atlasSaltCounter++) & 0xff;
  const host = document.createElement('div');
  host.style.width = '100%';
  host.style.height = '100%';

  const term = new Terminal({
    theme: saltTheme(theme, atlasSalt),
  ...
  entry.unsub.push(window.cth.onPtyData(ptyId, (rawChunk) => {
    ...
    term.write(chunk, () => {
      ...
      // The WebGL glyph atlas can corrupt SILENTLY during a fast TUI redraw
      // (spinners repainting via cursor-up + clear-line at full speed): cells
      // go blank or show the wrong glyph while the window stays focused, so
      // none of the loss/visibility triggers ever fire. There is no corruption
      // event to listen for, so while output streams we preventively drop the
      // atlas every few seconds — re-rastering the visible glyphs is cheap,
      // invisible, and puts a hard ceiling on how long any garbling survives.
      // (Scheduled off the write callback: clearing mid-render-frame is
      // exactly the kind of re-entrancy the addon dislikes.)
      maybeHealAtlas(entry, 4000);
    });
    entry.onData?.(chunk);
  }));
...
export function attachTerminal(entry: TerminalEntry, container: HTMLElement): void {
  ...
  // THE blank-white bug: leasing WebGL while the host is still 0×0 (attach
  // fires before layout, or under a hidden tab) creates the renderer's canvas
  // at 0×0 — and if the later fit() lands on the SAME cols/rows, no resize
  // event ever fires, the canvas stays 0×0, and the terminal renders blank
  // until something really changes the grid (the user toggling the queue
  // panel was "fixing" it by accident). So: wait for real dimensions, THEN
  // lease, then heal. Buffer and subscriptions run the whole time.
  const leaseWhenSized = (tries: number) => {
    if (!entry.host.isConnected) return; // detached before layout — next attach retries
    if (entry.host.clientWidth > 0 && entry.host.clientHeight > 0) {
      leaseWebglRenderer(entry);
      // Fresh atlas on every attach: re-parenting is when the silent-blank
      // strikes most; one lazy re-raster of the visible screen is cheap.
      maybeHealAtlas(entry, 0);
      return;
    }
    // Cap the wait (~2s at 60fps): a genuinely hidden host keeps its DOM
    // renderer until it shows; the sweep + next attach still cover it.
    if (tries > 0) requestAnimationFrame(() => leaseWhenSized(tries - 1));
  };
  leaseWhenSized(120);
